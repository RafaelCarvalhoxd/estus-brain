...
func TestClosingDateFor(t *testing.T) {
	...
	cases := []struct {
		...
		{"antes do fechamento", day(2026, time.September, 5), day(2026, time.September, 10)},
		{"no dia do fechamento fecha hoje", day(2026, time.September, 10), day(2026, time.September, 10)},
		{"depois do fechamento", day(2026, time.September, 15), day(2026, time.October, 10)},
		{"vira o ano", day(2026, time.December, 28), day(2027, time.January, 10)},
	}
...
func TestDueDateFor(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			day(2026, time.September, 10), day(2026, time.September, 20),
		},
		{
			"vencimento antes do fechamento vence no mês seguinte",
			CreditCard{ClosingDay: 25, DueDay: 15},
			day(2026, time.September, 25), day(2026, time.October, 15),
		},
		{
			"vencimento no mesmo dia do fechamento vence no mês seguinte",
			CreditCard{ClosingDay: 10, DueDay: 10},
			day(2026, time.September, 10), day(2026, time.October, 10),
		},
		{
			"vira o ano",
			CreditCard{ClosingDay: 25, DueDay: 15},
			day(2026, time.December, 25), day(2027, time.January, 15),
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
