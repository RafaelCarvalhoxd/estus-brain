// 3D simplex noise (Stefan Gustavson's algorithm, public domain), seeded so
// the brain's folds are the same shape on every load. Used once, when the
// brain geometry is built, to decide where the gyri rise and where the sulci
// fall — not per frame.
...
export function createNoise3D(seed = 1): (x: number, y: number, z: number) => number {
